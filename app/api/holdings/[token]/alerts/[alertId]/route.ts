...
// with the watchlist [alertId] route — see lib/alerts/routeHandlers.ts.
export async function PUT(request: Request, { params }: Context) {
  ...
  return handleAlertUpdate(HoldingAlerts, token, alertId, body)
}

export async function DELETE(_request: Request, { params }: Context) {
  const { token, alertId } = await params
  return handleAlertDelete(HoldingAlerts, token, alertId)
}
