...
// How often to pull a fresh live quote while the page is open. The cron job
// already refreshes snapshots once a minute; this tightens it to feel live for
// someone actively watching a group. Kept well above Angel One's per-minute
// quote budget so an open page can't rate-limit itself.
const LIVE_REFRESH_INTERVAL_MS = 15_000

type RefreshOutcome = { rateLimited: boolean }

// Single shared poller for the Strategy page. It drives the one expensive
// live fetch (Angel One quotes -> snapshots -> auto-close evaluation) and then
// invalidates every group card so they re-read the fresh price. Rendering it
// once — rather than per card — keeps concurrent group cards from each firing
// their own live fetch and tripping the rate limit.
export function StrategyLivePrices() {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<RefreshOutcome> => {
      ...
      // Fresh snapshots are persisted; pull every open group card forward.
      await queryClient.invalidateQueries({ queryKey: ['strategyGroup'] })
